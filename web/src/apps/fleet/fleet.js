  ...
  // command (see buildServerCmd). Like Sessions, hush only reads and composes —
  // it never starts or stops the server itself.
  function serverSection(m){
        ...
        if(url){
          return `<div class="oc-rt good">
            ...
            <div class="ocs-url"><code class="ocs-url-t">${esc(url)}</code>
              <button class="btn ocs-copy" data-url="${esc(url)}">Copy URL</button></div>
    ...
    return `<div class="sec-title">opencode server${servers.length?` <span class="n">${servers.length}</span>`:""}</div>
      <div class="oc-sec">
        ${body}
  ...
  function sessionsSection(m){
    ...
    return `<div class="sec-title">Sessions${list.length?` <span class="n">${list.length}</span>`:""}</div>
      <div class="se-sec">
        ${toolsStrip(m)}
        ${body}
        <div class="sheet-actions">
  ...
  function usersSection(m){
    ...
    return `<div class="sec-title">Users${list.length?` <span class="n">${list.length}</span>`:""}</div>
      <div class="us-sec">
        ${body}
        <div class="sheet-actions">
...
UNIT
  ...
  function enterMachine(id){ state.view="machine"; state.mid=id; render(); scrollTop();
    ...
    app.innerHTML = `
      ...
      <main>
        ...
        ${(()=>{
          ...
          return `<div class="sec-title">Inference <span class="n">${rts.length}</span></div>
            <div class="oc-sec">
              ${rows}
        ...
        ${(()=>{ const h = hostBackupEntry(m.id); const bks = (h&&h.backups)||[];
          return `<div class="sec-title">Backups${bks.length?` <span class="n">${bks.length}</span>`:""}</div>
        ...
        ${(()=>{ const inc = backupSourcesFor(m);
          ...
          return inc.length
            ? `<div class="sec-title">Receiving <span class="n">${inc.length}</span></div>
